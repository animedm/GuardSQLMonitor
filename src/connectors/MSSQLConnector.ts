...
export class MSSQLConnector {
  ...
  public async getMetricsForConnection(conn: ManagedConnection): Promise<DatabaseMetrics> {
    ...
    try {
      ...
      const responseTime = Date.now() - startTime;
      const connStats = connResult.recordset[0];
      const size = sizeResult.recordset[0];
      const cache = cacheResult.recordset[0] || { buffer_cache_hit_ratio: 0 };
      const resource = resourceResult.recordset[0] || {
        total_memory_mb: 1,
      ...
      return {
        ...
        responseTime,
        connections: {
          active: connStats.active_connections,
          idle: connStats.idle_connections,
          total: connStats.total_connections,
          max: 20,
          waiting: 0
        },
        performance: {
